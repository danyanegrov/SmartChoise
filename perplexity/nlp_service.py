...
class NLPProcessor:
    ...
    def _classify_intent(self, text: str) -> Tuple[str, float]:
        ...
        intent_patterns = {
            "purchase": [
                r'(купить|покупать|приобрести|нужен|нужна|нужно)',
                r'(хочу|желаю|планирую)\s+(купить|приобрести)',
                r'(подобрать|выбрать|найти).*(для\s+покупки|купить)'
            ],
            "compare": [
                r'(сравни|сравнить|сравнение)',
                r'(что\s+лучше|какой\s+лучше)',
                r'(отличия|разница|различия)',
                r'(vs|против|\sили\s)'
            ],
            "recommend": [
                r'(посоветуй|посоветовать|порекомендуй|рекомендация)',
                r'(что\s+выбрать|как\s+выбрать)',
                r'(лучший|оптимальный|подходящий)',
                r'(помоги\s+выбрать|помощь\s+в\s+выборе)'
            ],
            "information": [
                r'(расскажи|информация|узнать|что\s+это)',
                r'(характеристики|описание|детали)',
                r'(как\s+работает|принцип\s+работы)'
            ],
            "search": [
                r'(найди|найти|поиск|искать)',
                r'(где\s+купить|где\s+найти)',
                r'(магазин|shop|store)'
            ]
        }
    ...
    def _extract_filters(self, text: str, entities: List[Dict]) -> Dict[str, Any]:
        ...
        purpose_patterns = {
            'gaming': r'(игр|геймин|gaming|game)',
            'work': r'(работ|офис|бизнес|work|office)',
            'study': r'(учеб|студент|school|university)',
            'travel': r'(путешеств|отпуск|travel|vacation)'
        }
